...
public class AtomEventFramework implements AtomManager {
	...
	private HashMap<Atom,AtomEvent> activeAtoms;
	...
	public AtomEventFramework(Database db, ConfigBundle config) {
		...
		activeAtoms = new HashMap<Atom,AtomEvent>();
	}
	...
	public int checkToActivate() {
		...
		for (RandomVariableAtom atom : db.getAtomCache().getCachedRandomVariableAtoms()) {
			if (!activeAtoms.containsKey(atom) && atom.getValue() >= activationThreshold) {
				activated++;
				doActivateRVAtom(atom,AtomEvent.Type.ActivatedRVAtom);
			}
		}
		return activated;
	}
	...
	public void activateAtom(RandomVariableAtom atom) {
		...
		if (!activeAtoms.containsKey(atom)) {
			doActivateRVAtom(atom, AtomEvent.Type.ActivatedRVAtom);
		}
	}
	 ...
	 *                                       framework's Database
	 */
	public void activateAtom(RandomVariableAtom atom, AtomEvent.Type type) {
		if (!db.equals(atom.db))
			throw new IllegalArgumentException("Atom did not come from Database" +
					" managed by this AtomEventFramework.");
		if (!activeAtoms.containsKey(atom)) {
			doActivateRVAtom(atom,type);
		}
	}
	
	
	private void doActivateRVAtom(RandomVariableAtom atom, AtomEvent.Type type) {
		AtomEvent event = new AtomEvent(type,
				(RandomVariableAtom) atom, this);
		addAtomJob(event);
		activeAtoms.put(atom,event);
	}
	
	/**
	 * Adds a {@link AtomEvent#ActivatedObsAtom} event to the job queue for an
	 * ObservedAtom, regardless of its truth value.
	 * <p>
	 * No event will be generated if the Atom has already been activated by
	 * this AtomEventFramework since being loaded into memory.
	 * 
	 ...
	 *                                       framework's Database
	 */
	public void activateAtom(ObservedAtom atom) {
		if (!db.equals(atom.db))
			throw new IllegalArgumentException("Atom did not come from Database" +
					" managed by this AtomEventFramework.");
		if (!activeAtoms.containsKey(atom)) {
			doActivateObsAtom(atom,AtomEvent.Type.ActivatedObsAtom);
		}
	}
	
	
	private void doActivateObsAtom(ObservedAtom atom, AtomEvent.Type type) {
		AtomEvent event = new AtomEvent(type, atom, this);
		addAtomJob(event);
		activeAtoms.put(atom,event);
	}
	...
	public void workOffJobQueue() {
		...
		for (AtomEvent event : jobQueue)
			if (event.getType().equals(AtomEvent.Type.ActivatedRVAtom) ||
					event.getType().equals(AtomEvent.Type.ActivatedRVAtomOnline))
				((RandomVariableAtom)event.getAtom()).commitToDB();
	...
	private void notifyListeners(AtomEvent event) {
		...
		//log.trace("Notifying listeners about the great atom "+atom.toString());
